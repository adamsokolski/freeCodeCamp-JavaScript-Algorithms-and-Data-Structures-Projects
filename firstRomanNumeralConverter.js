...
function decimalToRoman(number, orderOfMagnitude) {
    ...
    if (indexNumber >= 1 && indexNumber <= 3) { // 1 - 3
        convertedNumber = letters[0].repeat(indexNumber)
    } else if (indexNumber === 4) { // 4
        convertedNumber = letters[0] + letters[1]
    } else if (indexNumber >= 5 && indexNumber <= 8) { // 5 - 8
        convertedNumber = letters[1]
        convertedNumber = convertedNumber.concat('', letters[0].repeat(indexNumber - 5))
    } else if (indexNumber === 9) { // 9
